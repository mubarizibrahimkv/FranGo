...
export default interface IcompanyService{
    ...
    getFranchises(companyId:string,page:number,search:string,filter?: Record<string, string>):Promise<Partial<{companyIndustryCategory:IIndustryCategory,franchises:IFranchise[],totalPages:number}>>
    ...
    getApplications(companyId:string,page:number,search:string,filter?:Record<string,string>):Promise<{application:IApplication[],totalPages:number}>
